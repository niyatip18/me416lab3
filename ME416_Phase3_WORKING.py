...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec  9 11:48:43 2025
...
# -*- coding: utf-8 -*-
"""
Multi-Robot Path Planner with Dynamic Obstacle Avoidance

Key additions for collision avoidance:
1. MQTT subscription to ALL other robots' positions
2. Dynamic obstacle map that includes other robots
3. Safety bubble around each robot
4. Real-time replanning when other robots enter path
5. Emergency stop if collision imminent
"""
...
class Config:
    ...
    SAFETY_MARGIN = 0.3  # still used conceptually; main bubble is AVOIDANCE_RADIUS
    COLLISION_AVOIDANCE_DISTANCE = 0.8   # start slowing down when inside this distance
    EMERGENCY_STOP_DISTANCE = 0.45       # hard-stop radius
...
def get_robot_pose_mqtt(mqtt_data, cfg, prev_pose, dt):
    ...
    Coordinates transformed into arena frame.
    """
    try:
...
# =============================================================================
# A* PLANNER WITH DYNAMIC OBSTACLES (UPDATED!)
# =============================================================================
        ...
        for ix in range(self.x_width):
            ...
            for iy in range(self.y_width):
                ...
                for ox, oy in dynamic_obstacles:
                    d = math.hypot(ox - x, oy - y)
                    # Use rr directly as avoidance radius (0.45 m)
                    if d <= self.rr:
                        obstacle_map[ix][iy] = True
                        break
...
class SingleRobotController:
    ...
    def plan_path(self, start_x, start_y):
        """
        Plan A* path from current pose to goal.
        For crowded scenarios: only avoid NEARBY robots during planning.
        """
        if self.goal_x is None:
            return False
        
        print("[CTRL] Planning A* path with dynamic obstacles...")
        
        # Get current positions of other robots
        ...
        # Decide dynamic obstacles for planner
        ...
        # Use AVOIDANCE_RADIUS as robot radius for "blocked" area
        planner = AStarPlanner(static_obstacles, dynamic_obstacles,
                               self.cfg.GRID_SIZE,
                               self.cfg.AVOIDANCE_RADIUS, bounds)
...
def run_single_limo(simulation=False):
    ...
    if simulation:
        ...
        print("Waiting for MoCap data on rb/limo{} ...".format(cfg.LIMO_NUMBER))
        t0 = time.time()
        while True:
            pose, valid = get_robot_pose_mqtt(mqtt_pose, cfg, None, 0.0)
            if valid:
                print("✓ Got initial MoCap pose:", pose)
                prev_pose = pose
                break
            if time.time() - t0 > 10.0:  # 10 second timeout
                print("No MoCap data after 10s, aborting.")
                return
            time.sleep(0.05)
    ...
    path_line, = ax.plot([], [], 'b--', linewidth=1.5, label="A* path")
    ...
    try:
        while True:
            ...
            if (x < cfg.ARENA_X_MIN - 0.1 or x > cfg.ARENA_X_MAX + 0.1 or
                ...
                print("[SAFETY] Robot outside arena bounds! Steering back in.")

                # Compute a point in the middle of the arena
                center_x = 0.5 * (cfg.ARENA_X_MIN + cfg.ARENA_X_MAX)
                center_y = 0.5 * (cfg.ARENA_Y_MIN + cfg.ARENA_Y_MAX)

                # Turn toward the center
                angle_to_center = math.atan2(center_y - y, center_x - x)
                heading_error = wrap_to_pi(angle_to_center - theta)

                # Small forward speed, strong turning to face inward
                v_cmd = 0.15
                omega_cmd = np.clip(2.0 * heading_error,
                                    -cfg.MAX_ANGULAR_VEL,
                                    cfg.MAX_ANGULAR_VEL)

                # Apply corrective command and skip normal logic this cycle
                if simulation:
                    ...
                    if tcp_socket:
                        try:
                            ...
                            print("TCP send failed while steering back in.")
                            break
                ...
                heading_arrow = ax.arrow(pose_plot[0], pose_plot[1],
                                         dx_vec, dy_vec,
                                         head_width=0.08, head_length=0.08,
                                         fc='black', ec='black', zorder=4)

                ax.set_title(f"State: SAFETY-RETURN | t = {t:.1f}s")
                plt.pause(0.001)
